...
users.forEach(user => {
  ...
  const result = userStmt.run(user.username, hashedPassword, user.full_name, user.role, user.email, user.phone);
  userIds[user.username] = result.lastInsertRowid || db.prepare('SELECT id FROM users WHERE username = ?').get(user.username).id;
});

const patientStmt = db.prepare(`
  INSERT OR IGNORE INTO patients (user_id, national_id, date_of_birth, address, medical_history, allergies)
  VALUES (?, ?, ?, ?, ?, ?)
`);

patientStmt.run(userIds['patient1'], '1234567890', '1990-05-15', 'الرياض، حي النخيل', 'لا يوجد', 'حساسية من البنسلين');
const patient1Id = db.prepare('SELECT id FROM patients WHERE user_id = ?').get(userIds['patient1']).id;

patientStmt.run(userIds['patient2'], '0987654321', '1985-08-22', 'جدة، حي الزهراء', 'ضغط الدم', 'لا يوجد');
const patient2Id = db.prepare('SELECT id FROM patients WHERE user_id = ?').get(userIds['patient2']).id;
...
medications.forEach(med => {
  medStmt.run(med.name, med.description, med.unit, med.quantity_in_stock, med.minimum_quantity, med.unit_price, med.category);
});
...
notifStmt.run('low_stock', 'تنبيه: نفاذ كمية', 'الدواء إيبوبروفين 400mg أوشك على النفاذ. الكمية المتبقية: 8', 2);
notifStmt.run('low_stock', 'تنبيه: نفاذ كمية', 'قفازات طبية أوشكت على النفاذ. الكمية المتبقية: 3', 4);
